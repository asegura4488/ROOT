...
// RooStats hypotheiss tests calculators and test statistic.
//
//
//Author:  L. Moneta
//
...
//
//
//"fileName" = Name of the file where the data with which the hypothesis test will be carried out.
//"workspace name" = Name of the workspace in the filename that contains the Nombre del workspace o que se va a usar en el programa, o que se uso en "S+B modelconfig name"
//"S+B modelconfig name" = Name of the modelConfig file that contains the model of signal+background, the one genereated using HistFactory
//"B model name" = Name of the modelConfig file that contains the model of background, it was not created previously. It is generated by the script by setting the parameter mu=0.
//"data set name" = Name assigne to the data set within the modelConfig. It is usually obsData using HistFactory
...
////////////////////////////////////////////////////////////////////////////////////////////////
/////     THIS SCRIPT HAS BEEN SLIGHTLY MODIFIED TOWARDS THE END TO STORE THE RESULTS IN A .root FILE
...
double poiValue = -1;                    // change poi snapshot value for S+B model (needed for expected p0 values)   POI mean Parmaters of Interest
int  printLevel=0;
...
*/
  ...
  if (!bModel ) {
   ...
   if (calcType != 2) {
    ...
    Double_t p_value, significance_t;

    
    if(newHypoTest == 0){
        
        //TNtuple *resultsHypoTest = new TNtuple("resultsHypoTest", "resultsHypoTest", "p_value:significance_t");
	TNtuple *resultsHypoTest = new TNtuple("resultsHypoTest", "resultsHypoTest", "p_value");
        
        //Store the current results
        resultsHypoTest->Fill(htr->NullPValue()) ;
